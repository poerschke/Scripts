...
void fatal(char *);
void normaliza(char *string);
int socket_connect(char *host, in_port_t port);
void head(char *host, int port, char *server);
void parser2(char *string);
void grava(char arquivo[], char ip[], char porta[], char server[]);
...
int main(int argc, char *argv[])
{
    ...
    sleep(30);
        ...
        FILE *log = fopen("scan.log", "r");


        arquivo = fopen("server.txt", "a");
        while(fgets(line, sizeof(line), log)){
                strtok(line, "\n");
                head(line, atoi(argv[2]), server);
                if(strlen(line) > 6){
                        fprintf(arquivo, "http://%s:%s/|%s\n", line, argv[2], server);
                        printf("http://%s:%s/|%s\n", line, argv[2], server );
                        if(strstr(server, "Desconhecido") != NULL)
                                grava("desconhecido.txt", line, argv[2], server);
                        else if(strstr(server, "micro_httpd") != NULL)
                                grava("micro_httpd.txt", line, argv[2], server);
                        else if(strstr(server, "nginx") != NULL)
                                grava("nginx.txt", line, argv[2], server);
                        else if(strstr(server, "Microsoft-IIS") != NULL)
                                grava("ms.txt", line, argv[2], server);
                        else if(strstr(server, "lighttpd") != NULL)
                                grava("lighttpd.txt", line, argv[2], server);
                        else if(strstr(server, "Apache") != NULL)
                                grava("Apache.txt", line, argv[2], server);
                        else if(strstr(server, "mini_httpd") != NULL)
                                grava("mini_httpd.txt", line, argv[2], server);
                        else
                                grava("outros.txt", line, argv[2], server);
...
Função para inicializar os MAX_SOCKETS definidos
...
Função para checar o status dos sockets
e escrver no arquivo de log os ips com
a porta aberta.
...
e finalizar os socket caso o endereço
IP seja inválido
